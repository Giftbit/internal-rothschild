...
async function executeTransactionPlans(auth: giftbitRoutes.jwtauth.AuthorizationBadge, trx: Knex, plans: TransactionPlan[], options: ExecuteTransactionPlannerOptions): Promise<Transaction[]> {
    ...
    try {
        for (plansIndex = 0; plansIndex < plans.length; plansIndex++) {
            insertedTransactions.push(await executeTransactionPlanWithRollback(auth, trx, plans[plansIndex], options));
        }
    } catch (err) {
        log.warn("Error thrown executing transaction plan.", err);
...
// todo - nuke
export async function executeTransactionPlansOLD(auth: giftbitRoutes.jwtauth.AuthorizationBadge, plans: TransactionPlan[]): Promise<Transaction[]> {
...
export async function executeTransactionPlanWithRollback(auth: giftbitRoutes.jwtauth.AuthorizationBadge, trx: Knex, plan: TransactionPlan, options: ExecuteTransactionPlannerOptions): Promise<Transaction> {
    auth.requireIds("userId", "teamMemberId");
