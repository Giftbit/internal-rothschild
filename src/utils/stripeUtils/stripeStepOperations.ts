...
import {getMerchantStripeAuth} from "./stripeAccess";
import log = require("loglevel");

export async function executeStripeSteps(auth: giftbitRoutes.jwtauth.AuthorizationBadge, plan: TransactionPlan): Promise<void> {
    const stripeSteps = plan.steps.filter(step => step.rail === "stripe") as StripeTransactionPlanStep[];
    if (!stripeSteps.length) {
        return;
    }

    const merchantStripeAuth = await getMerchantStripeAuth(auth);
    ...
    try {
        for (const step of stripeSteps) {
            ...
            if (step.type === "charge") {
                ...
                step.captureResult = await captureCharge(step.chargeId, captureParams, auth.isTestUser(), merchantStripeAuth.stripe_user_id);
            } else {
...
export async function rollbackStripeChargeSteps(auth: giftbitRoutes.jwtauth.AuthorizationBadge, steps: StripeChargeTransactionPlanStep[], reason: string): Promise<Stripe.refunds.IRefund[]> {
    const merchantStripeAuth = await getMerchantStripeAuth(auth);
    ...
    for (const step of steps) {
        try {
            ...
            const refund = await createRefund(refundParams, auth.isTestUser(), merchantStripeAuth.stripe_user_id);
...
function getLightrailTransactionSourcesSummary(currentStep: StripeChargeTransactionPlanStep, plan: TransactionPlan): string {
    let summary = JSON.stringify(
        plan.steps.filter(step => step !== currentStep || !(step.rail === "stripe" && step.type === "charge" && step.stepIdempotencyKey === currentStep.stepIdempotencyKey))
            .map(step => {
                switch (step.rail) {
                    case "lightrail":
                        return {
