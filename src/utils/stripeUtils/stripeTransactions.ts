        ...
        switch (err.type) {
            ...
            case "StripeCardError":
                if (isIdempotentReplayError(err)) {
                    let count = 1;
                    let nextStepIdempotencyKeyAndCount = getRetryIdempotencyKey(stepIdempotencyKey, count);
                    count = nextStepIdempotencyKeyAndCount.count;
                    if (count < 5) {
                        let newKey = nextStepIdempotencyKeyAndCount.newKey;
                        return await createCharge(params, isTestMode, merchantStripeAccountId, newKey);
                    }
                }
            ...
            case "StripeInvalidRequestError":
                ...
                throw new StripeRestError(cassava.httpStatusCode.clientError.UNPROCESSABLE_ENTITY, "Your request has invalid parameter", "StripeInvalidRequestError", err);
            default:
                throw err;
        }
    }
...
function checkForStandardStripeErrors(err: any): void {
    switch (err.type) {
        ...
        case "StripeConnectionError":
            throw new StripeRestError(502, "Stripe connection timeout", "StripeConnectionError", err);
        case "StripeAPIError":
            throw new StripeRestError(502, "Stripe's API is not responding.", "StripeAPIError", err);
        default:
            // do nothing
    }
}

function getRetryIdempotencyKey(stepIdempotencyKey: string,  count: number): {newKey: string, count: number} {
