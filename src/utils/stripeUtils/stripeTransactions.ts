        ...
        switch (err.type) {
            ...
            case "StripeCardError":
                if (isIdempotentReplayError(err)) {
                    const count = 1;
                    const nextStepIdempotencyKeyAndCount = getRetryIdempotencyKeyAndCount(stepIdempotencyKey, count);
...
function getRetryIdempotencyKeyAndCount(stepIdempotencyKey: string,  count: number): {newKey: string, count: number} {
